...
const mint = async (targetPublicKey, amount) => {
    try {
        ...
        const mintPublicKey = new web3.PublicKey(process.env.EURCH_PUBLIC_KEY);
        if (!mintPublicKey) throw new Error('Missing EURCH_PUBLIC_KEY in environment variables');
        ...
        const mintToTransaction = new web3.Transaction().add(
            spl.createMintToInstruction(
                ...
                amount * (10 ** 6) // Convert to the smallest unit (3 decimals)
            )
        );
